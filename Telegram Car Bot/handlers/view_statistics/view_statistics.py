...
async def fuel(callback: types.CallbackQuery, state: FSMContext):
    ...
    path = await plot_col_2_over_time_and_save(ans, 'id', 'consumption', 'C:\\Telegram Car Bot\\Data\\file.png') 
...
@router.callback_query(F.data == "parts_view_")   
async def fuel(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.delete()
    builder = InlineKeyboardBuilder()
...
@router.callback_query(F.data == "other_note_view_")   
async def fuel(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.delete()
    builder = InlineKeyboardBuilder()
